...
echo [6/7] 构建 portrait_animator ...
pyinstaller portrait_animator.spec --noconfirm
if %errorlevel% neq 0 (echo [失败] portrait_animator 构建失败 & exit /b 1)
echo [完成] portrait_animator
echo.

:: ---- 构建 Electron 安装包 ----
echo [7/7] 构建 Electron 安装包 ...
cd /d "%ROOT%app"
call npm run build:win
if %errorlevel% neq 0 (echo [失败] Electron 安装包构建失败 & exit /b 1)
echo.
echo ========================================
echo   构建完成！
